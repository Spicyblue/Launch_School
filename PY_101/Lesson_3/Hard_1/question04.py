...
def is_dot_separated_ip_address(input_string):
    dot_separated_words = input_string.split(".")
    if  len(dot_separated_words) !=4:  # ensure only list of four elements are allowed
        return False  
    while len(dot_separated_words) > 0:
        word = dot_separated_words.pop()
        if not is_an_ip_number(word):
            return False                # return false if anything besides a number is added.
...
print(is_dot_separated_ip_address('101.250.34.34')) # True
print(is_dot_separated_ip_address('101.acv.34.34')) # False
print(is_dot_separated_ip_address('101.250.34')) # False
print(is_dot_separated_ip_address('101.250.34.34.303')) # False